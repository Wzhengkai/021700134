...
def gettelnum(s):       #获得电话号码
    res = re.search(r'\d{11}', s)       #\d{11}表\d匹配11次
    if res == None:
        return ""
    return res.group(0)

def getprovince(s):      ##获得省
    res = re.search(("(.*?省)|(.*?自治区)|上海市|北京市|天津市|重庆市"), s) #使用.*?进行非贪心匹配
    if res != None:
        l = len(res.group(0))
    if res == None or l > 5:
        if s[0:3] == "黑龙江":    #
...
if PROVINCE in ('北京', '上海', '天津', '重庆'):
    ...
    NOWS = NOWS.replace(PROVINCE, "", 1)    #因这里已提取出完整的第一级地址了，所以删除第一级地址，缩短字符串
    PROVINCE = PROVINCE + "省"    #为普通的结尾没有“省”的第一级地址补全“省”
else:
    NOWS = NOWS.replace(PROVINCE, "", 1)    #由于PROVINCE以提取出来，所以删除province部分，缩短字符串
