...
def getprovince(is_eg):      ##获得省
    find = re.search(("(.*?省)|(.*?自治区)|上海市|北京市|天津市|重庆市"), is_eg) #使用.*?进行非贪心匹配
    if find != None:
        length = len(find.group(0))
    if find == None or length > 5:
        if is_eg[0:3] == "黑龙江":
            return "黑龙江"
        if is_eg[0:3] == "内蒙古":
            return "内蒙古"
...
def getcity(is_eg):            #获得城市
    find = re.search("(.*?自治州)|(.*?[市])", is_eg)
    if find != None:
        length = len(find.group(0))
    if find == None or length > 7:
        for i in CITIES:
            if is_eg[0:2] in i:
                return i
        return ""
...
def getcounty(is_eg):            #获得县
    find = re.search("(.*?自治旗)|(.*?[县区市旗])", is_eg)
    if find == None:
        for item in AREAS:
            if is_eg[0:2] in item:
                return item
        return ""
    return find.group(0)

def gettown(is_eg):         #获得城镇
    find = re.search("(.*?[镇乡口])|(.*?街道)", is_eg)
    if find == None:
        return ""
    return find.group(0)

def getroad(is_eg): #获得路
    res = re.search("(.*?[路街巷道])", is_eg)
    if res == None:
        return ""
    return res.group(0)

def getdoornum(is_eg):                         #获得门牌号
    find = re.search("(.*?[号])", is_eg)
    if find == None:
        return ""
    return find.group(0)
...
A = S[0]    #将表示等级的数字用A保存
S = S[2:]   #将等级后的部分重新赋给S
S_NEW = S.split(",")   #由于名字后立马跟着一个“，”，所以用逗号将总的地址分成两部分，逗号前的和逗号后的
NAME = S_NEW[0]    #逗号前的肯定为名字，所以可以提取出名字
NEW_STR = S_NEW[1]    #现在只要处理逗号后的部分，即除去名字外的地址信息
TELNUM = gettelnum(NEW_STR)    #调用gettelnum得到电话号码，由于电话号码都是11位的，所以用正则表达式匹配出连续的11个数字即可
NEW_STR = NEW_STR.replace(TELNUM, "", 1)  #由于已经匹配出电话号码了，所以可以将电话号码去掉，以缩短要处理的地址长度.参数1表替换次数不超过一次
NEW_STR = NEW_STR.replace(".", "", 1)     #把结尾句号再替换掉，也是为了缩短字符串
PROVINCE = getprovince(NEW_STR)        #处理省和直辖市，即处理第一级地址
if PROVINCE in ('北京', '上海', '天津', '重庆'):
    NEW_STR = NEW_STR.replace(PROVINCE, PROVINCE + "市", 1)    #因直辖市的第一级就是其本身，所以当PROVINCE提取出来后，就要手动为其增加市
elif PROVINCE in ('北京市', '上海市', '天津市', '重庆市'):
    PROVINCE = PROVINCE[0:2]    #提取第一级地址，即北京，上海，天津，重庆
elif PROVINCE == "广西":    #下面是对几个自治区的处理，处理名字
    NEW_STR = NEW_STR.replace(PROVINCE, "", 1)
    ...
    NEW_STR = NEW_STR.replace(PROVINCE, "", 1)    #因这里已提取出完整的第一级地址了，所以删除第一级地址，缩短字符串
    PROVINCE = PROVINCE + "省"    #为普通的结尾没有“省”的第一级地址补全“省”
else:
    NEW_STR = NEW_STR.replace(PROVINCE, "", 1)    #由于PROVINCE以提取出来，所以删除province部分，缩短字符串

CITY = getcity(NEW_STR)

if CITY != "":
    NEW_STR = NEW_STR.replace(CITY, "", 1)
    if (CITY[-1] != "市") and (CITY[-3:-1] != "自治"):
        CITY = CITY+"市"


COUNTY = getcounty(NEW_STR)
if COUNTY != "":
    L = len(COUNTY)
    if COUNTY[-1] == NEW_STR[L - 1]:
        NEW_STR = NEW_STR.replace(COUNTY, "", 1)
    else:
        NEW_STR = NEW_STR.replace(COUNTY[:-1], "", 1)

TOWN = gettown(NEW_STR)
NEW_STR = NEW_STR.replace(TOWN, "", 1)
DETAIADDR = NEW_STR

ROAD = getroad(NEW_STR)
NEW_STR = NEW_STR.replace(ROAD, "", 1)

DOORNUM = getdoornum(NEW_STR)
NEW_STR = NEW_STR.replace(DOORNUM, "", 1)
